...
def dim_counterparty(file):
    ...
    pq = df.to_parquet('df.parquet.gzip', compression='gzip',
                       storage_options=f"./")

    return 'Complete'

    # Place Dataframe into 'process' bucket:
    # response = s3.put_object(Body=dim_counterparty_obj,
    ...
    #     return {'status': 'Success', 'message': 'Object added!'}
    # else:
    #     return {'status': 'Failed', 'message': 'Object not added!', 'bucket_name': bucket_name, 'key': key, 'response': response}
