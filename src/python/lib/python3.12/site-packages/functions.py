...
def readFilesToProcess():
    client = boto3.client('ssm')
    files = client.get_parameter(Name='/backup')['Parameter']['Value']
    data = json.loads(files)
    return data


def writeFilesToProcess(filePath=0):
    client = boto3.client('ssm')
    # Call readBackup.  Returns JSON object:
    data = readFilesToProcess()
    # Access AWS Time. Create key name.
    keyName = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    # Set keyname and file path.
    data[keyName] = filePath
...
def put_hello():
    try:
        s3 = boto3.client('s3')
        bucket_name = get_bucket_names()['ingestion']
        string = 'Hello world!'
        s3.put_object(Body=string, Bucket=bucket_name, Key=string)
    ...
    "Records": [
        {
            ...
            "eventSource": "aws:s3",
            "awsRegion": "us-west-2",
            "eventTime": "2024-05-20T12:00:00.000Z",
            "eventName": "ObjectCreated:Put",
            "userIdentity": {
                "principalId": "AWS:123456789012:exampleUser@example.com"
            },
            "requestParameters": {
...
def process_write(event):
    try:
        s3 = boto3.client('s3')
        records = str(event['Records'])
        bucket_process = get_bucket_names()['process']
        s3.put_object(Key='record', Bucket=bucket_process)
        return records
...
# print(get_aws_time())
# new_date = datetime(2024, 4, 1, 12, 44, 0, 499417)
# update_aws_time(new_date)
# print(get_aws_time())
# data = all_data()
# print(data)
